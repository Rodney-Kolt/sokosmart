...
const TABS = [
  ...
  },
  {
    ...
    icon: (active) => (
      <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6">
        <path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" stroke={active ? "#f97316" : "#64748b"} strokeWidth="1.5" strokeLinecap="round" />
        <circle cx="12" cy="7" r="4" stroke={active ? "#f97316" : "#64748b"} strokeWidth="1.5" fill={active ? "rgba(249,115,22,0.1)" : "transparent"} />
      </svg>
    ),
  },
];
        ...
        {TABS.map((tab) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center gap-1 flex-1 py-2 transition-all duration-300 active:scale-95 relative min-h-[48px]"
            >
              <div className={`p-1.5 rounded-xl transition-all duration-300 ${isActive ? "bg-orange-500/10" : ""}`}>
                {tab.icon(isActive)}
                {badgeCount > 0 && (
              ...
              <span className={`text-[10px] font-medium transition-colors ${isActive ? "text-orange-400" : "text-slate-500"}`}>
                {tab.label}
              </span>
            </button>
          );
